...
//Categorías@#
var catnmb = 5,
    allCats = ['astronomia', 'biologia', 'curiosidades', 'fisica', 'tecnologia'];
var previewLim = 20; //Get search params
...
function initSrch(stAf) {
  ...
  kywords.forEach(function (itm, idx) {
    var str = itm.toLowerCase();
    str = rmDiacs(str);
    kywords.splice(idx, 1, str);
  });
  var all = true;

  for (var i = 0; i < catnmb; i++) {
  ...
  var _loop = function _loop(_i) {
    var itm = kywords[_i];
    if (itm == '' || itm == ' ') return "continue";
    var p = firebase.database().ref('searchQs/' + itm).transaction(function (search) {
      if (search) {
  ...
  for (var _i = 0; _i < kywords.length; _i++) {
    var _ret = _loop(_i);
...
function shwSrch() {
  ...
  srchRef.get().then(function (snap) {
    var docs = snap.docs;
    nxtp = false;
...
document.getElementById('catA').onclick = function () {
  if (!allChk) {
    ...
    for (var _i2 = 0; _i2 < catnmb; _i2++) {
      document.getElementById('cat' + _i2).checked = false;
    }
  }
...
for (var i = 0; i < catnmb; i++) {
  document.getElementById('cat' + i).onclick = function () {
    if (allChk) {
      ...
      for (var _i3 = 0; _i3 < catnmb; _i3++) {
        if (document.getElementById('cat' + _i3).checked == false) {
          all = false;
          break;
        }
      }
...
window.loaded = function loaded() {
  ...
  function send() {
    ...
    for (var _i4 = 1; _i4 < catnmb; _i4++) {
      if (document.getElementById('cat' + _i4).checked) {
        srchStr = srchStr + '+' + document.getElementById('cat' + _i4).value;
      }
    }

    srchStr = srchStr + '&k=';
    var wordArr = document.getElementById('srcBox').value.split(' ');
    srchStr = srchStr + wordArr[0];

    for (var _i5 = 1; _i5 < wordArr.length; _i5++) {
      srchStr = srchStr + '+' + wordArr[_i5];
    }
